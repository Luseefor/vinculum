...
**Default npm script** runs **`--project=chromium`** for fast local feedback. Full matrix locally: `npm run test:e2e:all-browsers` inside `apps/graph`.
...
1. Checkout, **Bun** install (`bun install --frozen-lockfile`).
2. **`bun run lint`** (workspace filter → graph app).
3. **`bun run typecheck`**.
4. **`bun run test`** (Vitest).
5. **`bun run build`** (production **`next build`**).
6. **Production server smoke**: after build, **`next start -p 3102`** in background, retry loop **`curl`** until `/` responds, basic grep on HTML payload, clean shutdown.
...
3. **`bun run dev`** → open the printed local URL (Next dev server).
4. Optional: **`cd apps/graph && npm run test:e2e:install`** then **`npx playwright install`** for firefox/webkit if running the full browser suite.
