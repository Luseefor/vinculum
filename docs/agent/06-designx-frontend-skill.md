...
  product classification, design system selection, aesthetic direction, hybrid design creation,
  device-aware sizing, professional UI copy discipline, layout archetypes, interaction design, user-flow architecture, frontend architecture, accessibility,
...
---
> **Vinculum integration note:** In the Vinculum agent pack, `00-agent-readme.md` remains the global project entrypoint. This file is the mandatory frontend/UI/design skill module. Use it for layout, interaction, responsive behavior, design-system selection, copy discipline, and frontend implementation quality. If this generic frontend skill conflicts with Vinculum-specific rules, `03-ui-ux-rules.md`, `01-architecture-contract.md`, and the actual codebase override it.
...
5. Select the aesthetic direction or intentionally merge compatible directions.
6. Select the device-aware density, typography scale, and sizing strategy.
7. Select the layout archetype.
8. Map the primary user flow.
9. Define navigation architecture.
10. Define screen states.
11. Define component architecture.
12. Define interaction and motion behavior.
13. Implement or specify the UI.
14. Run accessibility, responsiveness, performance, device-size, and visual QA.
15. Self-score. If any critical score is below 8/10, revise before final delivery.
```
...
- device-aware relative sizing
- no oversized text unless explicitly requested
...
→ Use Modules 1, 2, 3, 4, 5, 6, 7, 11, 13, 15, 16, 18, 19, 24.

USER ASKS FOR DASHBOARD / ANALYTICS
→ Use Modules 1, 7, 8, 14, 16, 18, 19, 24.

USER ASKS FOR DATA-HEAVY ADMIN / INTERNAL TOOL
→ Use Modules 1, 6, 7, 14, 16, 17, 18, 19, 24.

USER ASKS FOR MOBILE APP
→ Use Modules 2, 4, 5, 9, 10, 11, 18, 19, 24.

USER ASKS FOR CROSS-PLATFORM APP
→ Use Modules 2, 4, 9, 10, 11, 13, 18, 19, 24.

USER ASKS FOR REDESIGN
→ Use Modules 12, 3, 7, 11, 18, 19, 24. Audit first. Do not code first.
...
SIZE STRATEGY:  [e.g., “Compact desktop density, 14–16px body, restrained headings, fluid type clamp.”]
```

For small edits, you may use a shorter version.

## 0.7 Device-Aware Sizing Rule

Never make text, spacing, icons, cards, sidebars, or hero sections oversized by default. Large type is a deliberate stylistic choice, not a substitute for hierarchy. Before choosing sizes, think about the target device, viewport, use case, content density, and reading distance.

```txt
DEFAULT SIZE BEHAVIOR:
- Use restrained, proportional sizing unless the user asks for oversized, poster-like, brutalist, or landing-page hero typography.
- Prefer fluid and relative sizing: rem, em, %, fr, minmax(), clamp(), container queries, and content-based max-widths.
- Avoid fixed pixel-heavy layouts that only look good on one screen.
- A 1440px desktop layout, 1024px tablet layout, 768px tablet layout, 430px phone layout, and 375px small-phone layout must feel intentionally designed, not merely squeezed.
- Body text should usually live between 14px and 17px for application UI, depending on density and platform.
- H1/Hero text should scale with clamp() and should not dominate operational or data-heavy apps.
- Dense tools need smaller, precise type; marketing pages can use larger expressive type; mobile needs thumb-safe controls and readable text.
```

Before final delivery, ask internally: “Would this typography and spacing still feel correct on the actual device where the user will use it?”
...
## 3.0 Direction Creation and Hybrid Design Rules

The catalog is a set of reference directions, not a prison. The agent must be capable of creating different design languages and merging compatible directions when the product, platform, domain, or user request requires it.

```txt
CREATE A NEW DIRECTION WHEN:
- the product domain has a strong visual context, such as construction, robotics, finance, healthcare, music, education, logistics, gaming, or research
- the user provides brand cues that do not fit one catalog direction
- the existing UI needs a fresh identity rather than a known design-system look
- the platform requires conventions that conflict with the chosen aesthetic
- the design must balance multiple needs, such as enterprise density + consumer warmth
```

### Hybrid Direction Protocol

When merging styles, define the merge explicitly:

```txt
BASE STYLE:        [dominant direction controlling layout, density, typography]
BORROWED STYLE:    [secondary influence used only for selected traits]
BORROWED TRAITS:   [e.g., motion, material, icon style, accent color, border treatment]
PROTECTED RULES:   [what must not change, such as enterprise readability or mobile ergonomics]
REJECTION RULE:    [what is deliberately excluded to avoid visual confusion]
```

Good hybrids:

```txt
Industrial Carbon + Minimal Functional
→ dense enterprise construction UI with restrained orange accents, tables, split panes, and no decorative gradients.

Editorial Serif + Luxury Refined
→ premium publication/architecture site with serif hierarchy, quiet spacing, and restrained materials.

Data Terminal + Swiss Grid
→ developer analytics tool with dense monospaced data and strict alignment.

Soft Consumer + Material 3
→ friendly Android-first consumer app with rounded surfaces, expressive motion, and accessible navigation.
```

Bad hybrids:

```txt
✗ Cyberpunk + Luxury + Organic + Glassmorphism on one screen
✗ Enterprise admin panel with oversized marketing hero typography
✗ Mobile productivity app using desktop terminal density
✗ Healthcare app using neon/glitch motion for serious clinical workflows
```

### Different-Design Requirement

Do not repeatedly generate the same visual composition. Across tasks, vary the design by changing at least three of these:
...
If the current request resembles a common pattern, intentionally choose a different layout model unless the product type clearly demands the same one.
...
# MODULE 24 — DEVICE-AWARE SIZING, DENSITY, AND RELATIVE LAYOUT

A beautiful UI is not beautiful if it only works in one screenshot size. Always design for the actual device and use context. Typography, spacing, navigation, and layout density must change based on viewport, platform, interaction mode, and user task.

## 24.1 No Oversized Text by Default

Oversized text is allowed only when it serves the product. Do not use huge headlines, massive numbers, giant buttons, or excessive empty spacing to make the UI look “modern.”

```txt
USE LARGE TEXT WHEN:
- user explicitly requests bold, poster-like, hero, editorial, luxury, or brutalist design
- marketing page needs a strong above-the-fold message
- dashboard has one primary metric that should dominate
- mobile screen needs a single action or status to be instantly understood

AVOID LARGE TEXT WHEN:
- product is data-heavy
- screen is an admin/internal tool
- many controls must fit above the fold
- user needs comparison, scanning, or bulk operation
- mobile viewport would lose essential content
- large type causes wrapping, awkward truncation, or scrolling
```

## 24.2 Recommended Typography Ranges

These are defaults, not laws. Adjust based on product type and platform.
...
  table cells: 12–14px
  page title: 20–28px
  hero-style text: avoid unless requested

STANDARD SAAS / PRODUCTIVITY:
  body: 14–16px
  labels: 12–14px
  page title: 24–36px
...
  labels: 12–14px
  nav labels: 10–12px
  title: 20–34px depending on platform
  touch target: 44pt iOS / 48dp Android minimum
```

## 24.3 Relative Sizing Requirements

Use relative and fluid sizing so the UI adapts naturally.
...
/* Avoid */
font-size: 72px; /* without responsive fallback */
width: 1440px;
height: 900px;
margin-left: 237px;
```
...
```txt
- rem for type and spacing tied to text rhythm
- em for component-local scaling
- % and fr for fluid layout distribution
- min(), max(), clamp() for bounded responsiveness
- minmax() and auto-fit/auto-fill for grids
- container queries for component-level responsiveness
- viewport units only with safe bounds
```

## 24.4 Device and Context Checklist

Before delivering, evaluate the UI on these mental viewports:

```txt
375px small phone:
- Does primary content remain readable?
- Are touch targets large enough?
- Does text wrap awkwardly?
- Is navigation usable without hover?

430px large phone:
- Does the screen use space better than a simple stack?
- Are bottom actions reachable?

768px tablet:
- Should layout become two-pane, split-view, or rail-based?
- Does it avoid feeling like a stretched phone UI?

1024px small laptop/tablet landscape:
- Does navigation become persistent?
- Can tables and inspectors appear?

1280–1440px desktop:
- Is density appropriate?
- Is there wasted empty space?
- Are line lengths controlled?

1536px+ wide desktop:
- Does the layout use max-widths, columns, panes, or side inspectors instead of stretching endlessly?
```

## 24.5 Density Selection

Choose density based on the task, not aesthetics.
...
Compact does not mean cramped. It means smaller type, tighter row heights, precise spacing, clear alignment, and stronger information hierarchy.

## 24.6 Layout Must Respond by Behavior, Not Only Size

Responsive design is not merely shrinking widths. Components should change behavior.

```txt
Desktop table → tablet table with pinned first column → mobile row summaries with drill-in detail
Desktop sidebar → tablet rail → mobile bottom tabs or drawer
Desktop modal → mobile bottom sheet or full-screen sheet
Desktop hover menu → mobile tap/long-press menu
Desktop right inspector → mobile pushed detail screen
Desktop split pane → mobile stacked navigation flow
```

## 24.7 Visual Balance Rule

On every viewport, check:

```txt
- Does any text feel comically large?
- Does any component dominate without reason?
- Are controls proportional to content?
- Does the layout preserve enough information above the fold?
- Does the screen feel intentionally composed, not auto-scaled?
- Does the type scale create hierarchy without destroying density?
```

If the answer is no, revise sizing before final delivery.

---

# MODULE 25 — PROFESSIONAL UI COPY AND CONTENT DISCIPLINE

Frontend quality is not only layout and styling. The words visible in the interface must be professional, direct, relatable, and appropriate to the product. Do not inflate simple labels into dramatic or technical phrases. Good UI copy reduces cognitive load.

## 25.1 No Unwanted Text

Do not add unnecessary labels, descriptions, explanations, helper text, slogans, badges, subtitles, or decorative phrases just to fill space. Every word must help the user understand, decide, or act.

```txt
DO NOT ADD TEXT WHEN:
- the UI is already understandable without it
- the text repeats what the heading, icon, or button already says
- the phrase only makes the screen look busier
- the phrase sounds like marketing filler
- the phrase adds technical complexity for a normal user
- the phrase makes a simple area feel over-engineered
```

Prefer fewer, better words. Dense software should use concise labels. Consumer software should use natural language. Enterprise software should use precise business language without buzzwords.

## 25.2 Use Professional, Relatable Words

Use language that real users would expect inside a serious product. Avoid fake SaaS jargon, exaggerated labels, and AI-generated naming patterns.
...
AVOID UNLESS THE USER OR DOMAIN REQUIRES IT:
...
The label should match the user's mental model. If users call it a dashboard, call it Dashboard. If users call it reports, call it Reports. Do not make ordinary navigation sound like a sci-fi control panel.

## 25.3 Technical Language Restraint

Do not expose implementation or technical architecture in visible UI copy unless the product is explicitly for developers or technical operators.
...
- Permission-Aware Operational Surface
...
For developer tools, technical terms are allowed only when they are domain-correct and useful: Deployments, Builds, Logs, Branches, Commits, API Keys, Webhooks, Traces, Runtime, Queue, Schema, Tokens.

## 25.4 Label Length Rules

Short labels usually make better UI. Use the shortest clear label that preserves meaning.

```txt
NAVIGATION LABELS:
- 1–2 words preferred
- 3 words only when needed for clarity
- never use slogans as nav labels

BUTTON LABELS:
- use verb-first action text
- examples: Save, Create project, Export CSV, Invite member, Approve invoice
- avoid vague labels: Proceed, Utilize, Execute, Initiate, Leverage

SECTION HEADINGS:
- describe the content, not the design concept
- good: Recent activity, Budget status, Open issues
- bad: Operational Command Feed, Financial Intelligence Layer, Issue Resolution Matrix

EMPTY STATES:
- one plain headline
- one useful sentence
- one primary action
- no motivational fluff unless it matches the product
```

## 25.5 Do Not Over-Name Layout Areas

The agent must not invent dramatic names for standard interface regions.
...
- Strategic detail surface
- Data command table
- Operational awareness panel
```

Use terms like Command Center or Mission Control only when the user explicitly asks for that tone, the product domain genuinely supports it, or the selected aesthetic direction is intentionally terminal, military, cyberpunk, or operations-heavy.

## 25.6 Copy Must Match Product Type

```txt
CONSUMER APP:
- warm, simple, human
- example: “You have no saved items yet.”

ENTERPRISE / ADMIN:
- precise, neutral, action-oriented
- example: “No users match the current filters.”

CONSTRUCTION / FIELD OPERATIONS:
- practical, direct, job-site language
- example: “3 tasks need review before today’s closeout.”

FINANCE / ACCOUNTING:
- exact, conservative, audit-safe
- example: “This invoice is pending approval.”

DEVELOPER TOOL:
- technical but accurate
- example: “Build failed on step 4. View logs.”

MARKETING PAGE:
- persuasive, but not bloated
- example: “Manage projects, crews, budgets, and documents in one place.”
```

## 25.7 Interface Copy Review Gate

Before delivering, review all visible words:

```txt
□ Is every label necessary?
□ Is the wording clear to the target user?
□ Did I avoid inflated names like “Command Center” unless requested?
□ Did I avoid technical terms where plain words work better?
□ Are navigation labels short and familiar?
□ Are button labels action-oriented?
□ Are headings useful, not decorative?
□ Does the copy sound like a real product, not AI filler?
```

If any visible text sounds exaggerated, generic, overly technical, or unrelated to the user's domain, rewrite it before final delivery.
...
✗ oversized hero typography in operational/data-heavy apps unless requested
✗ giant cards, giant buttons, or giant whitespace used to fake premium design
✗ decorative icons with no meaning
✗ inflated UI labels like "Dashboard Command Center" when "Dashboard" is enough
✗ fake SaaS jargon, dramatic section names, or technical words in normal user-facing copy
✗ helper text, subtitles, slogans, or descriptions added only to fill empty space
...
✗ no layout archetype
✗ fixed screenshot-only layout that breaks on real devices
...
10. Device-Aware Sizing and Density Strategy
11. Implementation or Build Instructions
12. QA Checklist
```
